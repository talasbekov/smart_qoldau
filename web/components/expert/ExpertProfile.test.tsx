...
const REVIEWS = {
  items: [
    { rating: 5, publicText: 'Помогла разобраться.', expertReply: null, tags: ['attentive'], createdAt: '2026-08-01T10:00:00.000Z' },
  ],
...
describe('ExpertProfile', () => {
  it('имя специалиста — заголовок первого уровня', () => {
    ...
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('Айгуль Смагулова');
  });
  ...
  it('ведёт обратно в каталог', () => {
    ...
    expect(screen.getByRole('link', { name: /Все специалисты/ })).toHaveAttribute(
      'href',
      '/ru/catalog',
    );
  });
