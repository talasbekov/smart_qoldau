...
export default async function FavoritesPage({
  ...
  return (
    <>
      <h1 className="mb-6 text-2xl font-extrabold text-ink">Избранное</h1>

      {experts.length === 0 ? (
        <div className="py-12">
          <p className="mb-4 text-body">Здесь появятся специалисты, которых вы сохранили</p>
          <Link
            href={`/${locale}/catalog`}
        ...
        <ul className="grid grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-4">
          {experts.map((expert) => (
            <li key={expert.id} className="rounded-[20px] border border-border bg-white p-5">
