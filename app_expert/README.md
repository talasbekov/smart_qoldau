...
- ~~Ответ/жалоба на отзыв не реализованы (задача 15)~~ — **закрыто**:
  бэкенд завёл приватный `GET /experts/me/reviews` (`MyExpertReviewsDto`,
  items с `id`), экран отзывов переведён на него, кнопки
  «Ответить»/«Пожаловаться» работают. Публичный
  `GET /experts/{id}/reviews` остаётся полностью анонимным и `id` не
  несёт — это по-прежнему намеренно.
