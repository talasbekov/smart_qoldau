...
Запись к специалисту на конкретное время: экран выбора слота
(`features/booking`), перенос (`SlotPickerScreen` с `consultationId`) и
плановые карточки на вкладке «Активные».

Всё время показывается по Алматы, и это сказано на экране прямым текстом —
клиент из другого пояса иначе ошибётся на несколько часов. Бэкенд отдаёт
слоты в UTC, перевод живёт в одном месте (`state/slots_controller.dart`).

Диалог отмены предупреждает о счётчике отмен, если до начала меньше двух
часов: последствие должно быть известно ДО нажатия (Р-17).

**Ручной прогон на эмуляторе не выполнен:** в системе стоит только JRE,
`javac` отсутствует, поэтому сборка Android невозможна. Снимается
установкой `openjdk-21-jdk` — код и автотесты от этого не зависят.
