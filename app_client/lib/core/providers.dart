...
/// вызывать `socket.connect` тут не нужно.
///
/// **Бюджет [_authRetryBudget] расходуется ТОЛЬКО настоящими отказами
/// аутентификации** (Round 3 ревью, п.1 — до этой правки расходовался
/// БЕЗУСЛОВНО, ещё до того, как известен исход рефреша, включая чисто
/// сетевые сбои; после достаточного числа обычных обрывов связи это
/// доходило до `tokenStore.clear()` и разлогинивало пользователя с
/// нестабильной связью на ровном месте — та же беда, которую Round 2 уже
/// лечил с другой стороны). `401` на сам `/auth/refresh` (сам refresh-токен
/// мёртв, отозван или ротирован — единственный код, которым бэкенд отвечает
/// на это, см. `AuthService.refresh`) — это приговор и списывает одну
/// попытку бюджета; ЛЮБОЙ другой исход ([ApiErrorCode.network] — нет
/// сети/таймаут, 5xx, что угодно ещё) означает «попробуем позже» и не
/// трогает ни бюджет, ни хранилище токенов. Исчерпание бюджета — ПОДРЯД
/// идущие настоящие `401`-отказы без успешного `connected` между ними —
/// проверяется в начале функции ДО следующей попытки и вызывает
/// [onSessionInvalid] (тот же путь, что и принудительный логаут в
/// [sqApiProvider]): финальный сигнал приходит не в момент самого
/// последнего отказа, а на очередном `disconnected`, который эту попытку
/// уже не тратит.
///
  ...
  final connectionSub = socket.connectionState.listen((state) {
    ...
    unawaited(() async {
      ...
      try {
        ...
        await refresh();
        // Успех бюджет не трогает — это не отказ, а штатный исход.
      } on ApiException catch (e) {
        if (e.code != ApiErrorCode.unauthorized) {
          // Нет сети, таймаут, 5xx и всё прочее, кроме 401, — не отказ
          // аутентификации, а «попробуем позже»: бюджет не тратим, сессию
          // не чистим (Round 3 ревью, п.1).
          return;
        }
        // 401 на сам /auth/refresh — refresh-токен тоже мёртв, это
        // приговор. Списываем одну попытку; инвалидация наступит не
        // сейчас, а когда бюджет будет исчерпан (см. верхний if выше).
        authRetryBudget--;
      }
    }());
  });
