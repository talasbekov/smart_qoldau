...
void main() {
  ...
  group('SqApiExpertProfile.createExpert', () {
    test('createExpert отправляет корректное тело и разбирает ExpertMeDto', () async {
      dioAdapter.onPost(
        '/experts',
    ...
    test('createExpert при PRICE_OUT_OF_RANGE пробрасывает ApiException', () async {
      dioAdapter.onPost(
        '/experts',
        (server) => server.reply(400, {
          'error': {'code': 'PRICE_OUT_OF_RANGE', 'message': 'Цена вне диапазона'}
        }),
        data: {
          'displayName': 'Айгуль Серикова',
          'city': 'Алматы',
    ...
    test('createExpert при EXPERT_EXISTS пробрасывает ApiException', () async {
      dioAdapter.onPost(
        ...
        (server) => server.reply(409, {
          'error': {'code': 'EXPERT_EXISTS', 'message': 'Эксперт уже существует'}
        }),
        data: {
          'displayName': 'Айгуль Серикова',
          'city': 'Алматы',
  ...
  group('SqApiExpertProfile.me', () {
    test('me возвращает текущий профиль эксперта', () async {
      dioAdapter.onGet('/experts/me', (server) => server.reply(200, expertMeJsonFixture));
    ...
    test('me при NOT_VERIFIED пробрасывает ApiException', () async {
      dioAdapter.onGet('/experts/me', (server) => server.reply(403, {
        'error': {'code': 'NOT_VERIFIED', 'message': 'Профиль не верифицирован'}
      }));
      expect(
        () => api.me(),
        throwsA(isA<ApiException>().having(
  ...
  group('SqApiExpertProfile.updateExpert', () {
    test('updateExpert отправляет обновленные данные и возвращает ExpertMeDto', () async {
  ...
  group('SqApiExpertProfile.setWorkStatus', () {
    test('setWorkStatus отправляет статус работы и возвращает ExpertMeDto', () async {
      final updatedFixture = {
        ...expertMeJsonFixture,
        'workStatus': 'BUSY',
      };
      dioAdapter.onPatch('/experts/me/work-status', (server) => server.reply(200, updatedFixture),
          data: {'status': 'BUSY'});
  ...
  group('SqApiExpertProfile.heartbeat', () {
    test('heartbeat отправляет POST без тела', () async {
      dioAdapter.onPost('/experts/me/heartbeat', (server) => server.reply(204, null));
      await api.heartbeat();
    });
  });
}
