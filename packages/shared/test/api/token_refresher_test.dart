...
void main() {
  group('TokenRefresher', () {
    test(
      'читает текущий refresh-токен, вызывает refreshCall и пишет результат',
      () async {
        ...
        expect(
          current.accessToken,
          'new-access',
          reason: 'результат должен быть записан через writer',
        );
      },
    );

    test('параллельные вызовы refresh() делят один и тот же refreshCall '
        '(single-flight)', () async {
      var current = _tokens('old-access', 'old-refresh');
    ...
    test('после завершения обновления следующий refresh() запускает новый refreshCall', () async {
      ...
      expect(
        ...
        2,
        reason:
            '_inFlight обязан очищаться после завершения — иначе второй '
            'вызов навсегда получал бы устаревший первый результат',
      );
    });

    test(
      'провал refreshCall пробрасывается вызывающему и НЕ пишет токены',
      () async {
        var writeCalls = 0;
        final refresher = TokenRefresher(
    ...
    test('после провала следующий refresh() пробует снова, а не застревает '
        'на отклонённом _inFlight', () async {
      var attempt = 0;
      final refresher = TokenRefresher(
        () async => _tokens('old-access', 'old-refresh'),
        (_) async {},
