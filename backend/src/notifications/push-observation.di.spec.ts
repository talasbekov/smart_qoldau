...
const TEST_ENV: Record<string, string> = {
  ...
  JWT_SECRET: 'push-observation-di-secret-32-chars',
...
describe('Push observation production DI', () => {
  ...
  it('resolves the registered controller and read service from AppModule', () => {
    expect(moduleRef.get(NotificationsAdminController)).toBeInstanceOf(
      NotificationsAdminController,
    );
    expect(moduleRef.get(PushObservationService)).toBeInstanceOf(
