...
describe('bootstrap trusted proxy boundary', () => {
  ...
  async function send(
    ...
    // Exercise Express's real request getters and the production guard without
    // opening a socket. Socket peer values are transport fixtures; the separate
    // HTTP suite verifies them over real IPv4-mapped IPv6 connections.
    const expressApp = app.getHttpAdapter().getInstance() as Application;
    const req = Object.assign(Object.create(express.request), {
      app: expressApp,
      socket: { remoteAddress: `::ffff:${peer}` },
      headers: xff === undefined ? {} : { 'x-forwarded-for': xff },
    }) as Request;
    if (path === 'ip')
      return { status: 200, body: new ProbeController().ip(req) };
    const res = { header: () => undefined };
    const context = new ExecutionContextHost(
      [req, res],
      ProbeController,
      ProbeController.prototype.limited,
    );
    try {
      await app.get(IpThrottlerGuard).canActivate(context);
      return { status: 200, body: { ok: true } };
    } catch (error) {
      return {
        status: error.getStatus(),
        body: { error: error.getResponse() },
      };
    }
  }
