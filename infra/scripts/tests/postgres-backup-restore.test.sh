...
for argument in "$@"; do
  case "$argument" in
    *pg_cancel_backend*)
      mode="$(cat /tmp/smartqoldau-test-cancel-mode)"
      printf '%s\n' "$mode" >>/tmp/smartqoldau-test-cancel-observed
      case "$mode" in
        exit77) exit 77 ;;
        false0) printf 'f\n'; exit 0 ;;
        multiple0)
          printf 'SMARTQOLDAU_CANCELLED\nSMARTQOLDAU_CANCELLED\n'
          exit 0
          ;;
        *) exit 78 ;;
      esac
...
run_cancel_refusal_case() {
  local mode="$1"
  local safe_mode="${mode//[^a-zA-Z0-9_]/_}"
  local lock_application_name="smartqoldau_test_${safe_mode}_lock_${RUN_ID//-/_}"
  local refusal_backup="$WORK_DIR/cancel-refusal-$safe_mode"
  local refusal_log="$WORK_DIR/cancel-refusal-$safe_mode.log"
  local refusal_signal_status refusal_owned_backend_count refusal_owned_clients
  local refusal_control_leftovers observed_mode

  docker exec "$SOURCE_CONTAINER_ID" sh -ceu \
    'printf "%s\n" "$1" >/tmp/smartqoldau-test-cancel-mode; : >/tmp/smartqoldau-test-cancel-observed' \
    sh "$mode"

  docker exec --env "PGAPPNAME=$lock_application_name" "$SOURCE_CONTAINER_ID" \
    psql --no-psqlrc --set ON_ERROR_STOP=1 --username fixture_user --dbname fixture_db \
      --command 'BEGIN; LOCK TABLE backup_parent IN ACCESS EXCLUSIVE MODE; SELECT pg_sleep(30);' \
    >"$WORK_DIR/refusal-lock-$safe_mode.log" 2>&1 &
  LOCK_EXEC_PID=$!
  wait_for_query_count "$SOURCE_CONTAINER_ID" \
    "SELECT count(*) FROM pg_stat_activity WHERE application_name = '$lock_application_name' AND wait_event = 'PgSleep'" \
    || fail "$mode lock query did not become active"

  "$BACKUP_SCRIPT" \
    --container "$SOURCE_CONTAINER_ID" --host 127.0.0.1 --port 5432 \
    --database fixture_db --user fixture_user --password-file "$PASSWORD_FILE" \
    --output "$refusal_backup" >"$refusal_log" 2>&1 &
  BACKUP_PROCESS_PID=$!
  wait_for_query_count "$SOURCE_CONTAINER_ID" \
    "SELECT count(*) FROM pg_stat_activity WHERE application_name LIKE 'smartqoldau_backup_%'" \
    || fail "$mode pg_dump did not become visible"

  kill -TERM "$BACKUP_PROCESS_PID"
  wait_for_process_exit "$BACKUP_PROCESS_PID" 10 \
    || fail "$mode backup did not exit within 10 seconds after TERM"
  set +e
  wait "$BACKUP_PROCESS_PID"
  ...
  if [[ "$refusal_signal_status" != '143' ]]; then
    sed -n '1,160p' "$refusal_log" >&2
    fail "$mode backup TERM status was $refusal_signal_status, expected 143"
  fi

  observed_mode="$(docker exec "$SOURCE_CONTAINER_ID" cat /tmp/smartqoldau-test-cancel-observed)"
  [[ "$observed_mode" == "$mode" ]] || fail "$mode fixture did not exercise the intended cancel result"
  [[ "$(docker container inspect --format '{{.State.Running}}' "$SOURCE_CONTAINER_ID")" == 'true' ]] \
    || fail "$mode fallback stopped the source container"
  wait_for_query_count "$SOURCE_CONTAINER_ID" \
    "SELECT count(*) FROM pg_stat_activity WHERE application_name = '$lock_application_name' AND wait_event = 'PgSleep'" \
    || fail "$mode fallback stopped an unrelated source query"
  ...
  [[ "$refusal_owned_backend_count" == '0' ]] || fail "$mode fallback left its PostgreSQL backend running"
  ...
  ')"
  [[ -z "$refusal_owned_clients" ]] || fail "$mode fallback left owned client processes: $refusal_owned_clients"
  ...
  [[ -z "$refusal_control_leftovers" ]] \
    || fail "$mode fallback left credential/control files: $refusal_control_leftovers"
  [[ ! -e "$refusal_backup" ]] || fail "$mode backup published a final artifact"
  if find "$WORK_DIR" -maxdepth 1 -name ".cancel-refusal-$safe_mode.tmp.*" -print -quit | grep -q .; then
    fail "$mode backup left a temporary directory"
  fi
  ...
  wait "$LOCK_EXEC_PID" 2>/dev/null || true
  LOCK_EXEC_PID=''
  docker exec "$SOURCE_CONTAINER_ID" rm -f -- \
    /tmp/smartqoldau-test-cancel-mode /tmp/smartqoldau-test-cancel-observed
}

run_cancel_refusal_case exit77
run_cancel_refusal_case false0
run_cancel_refusal_case multiple0
docker exec "$SOURCE_CONTAINER_ID" sh -ceu '
  psql_path="$1"
  rm -f -- "$psql_path"
  mv -- "${psql_path}.smartqoldau-real" "$psql_path"
' sh "$PSQL_PATH"
printf 'PASS: exact-process fallback after unconfirmed SQL cancel\n'
